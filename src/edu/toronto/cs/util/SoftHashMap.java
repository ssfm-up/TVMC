 ...
 */
public class SoftHashMap extends HashMap implements Map
{
   ...
   */
  private final class SoftEntrySet extends AbstractSet
  {
    /**
    ...
    public Iterator iterator()
    {
      return new Iterator()
      {
         ...
         * to the key.
         */
        SoftBucket.SoftEntry lastEntry;
         ...
         * to the key.
         */
        SoftBucket.SoftEntry nextEntry = findNext(null);
         ...
         */
        private SoftBucket.SoftEntry findNext(SoftBucket.SoftEntry lastEntry)
        {
          int slot;
          SoftBucket nextBucket;
          if (lastEntry != null)
            {
          ...
          while (true)
            {
              while (nextBucket != null)
                {
                  SoftBucket.SoftEntry entry = nextBucket.getEntry();
                  if (entry != null)
   ...
   * @author Jochen Hoenicke
   */
  private static class SoftBucket extends SoftReference
  {
    /**
     ...
     *        will be enqueued.
     */
    public SoftBucket(Object key, ReferenceQueue queue, Object value,
                      int slot)
    {
      super(key, queue);
      this.value = value;
    ...
    class SoftEntry implements Map.Entry
    {
       ...
       */
      public SoftBucket getBucket()
      {
        return SoftBucket.this;
      }
   ...
   */
  private final SoftEntrySet theEntrySet;
  ...
  public SoftHashMap(int initialCapacity, float loadFactor)
  {
    ...
    theEntrySet = new SoftEntrySet();
    queue = new ReferenceQueue();
    buckets = new SoftBucket[initialCapacity];
  }
  ...
  void cleanQueue()
  {
    ...
    while (bucket != null)
      {
        internalRemove((SoftBucket) bucket);
        bucket = queue.poll();
      }
  }
   ...
   */
  private SoftBucket.SoftEntry internalGet(Object key)
  {
    if (key == null)
      key = NULL_KEY;
    int slot = hash(key);
    SoftBucket bucket = buckets[slot];
    while (bucket != null)
      {
        SoftBucket.SoftEntry entry = bucket.getEntry();
  ...
  private void internalAdd(Object key, Object value)
  {
    if (key == null)
      key = NULL_KEY;
    int slot = hash(key);
    SoftBucket bucket = new SoftBucket(key, queue, value, slot);
    bucket.next = buckets[slot];
    buckets[slot] = bucket;
    size++;
  }
   ...
   */
  void internalRemove(SoftBucket bucket)
  {
  ...
  {
    cleanQueue();
    SoftBucket.SoftEntry entry = internalGet(key);
