...
public class Lift extends SMVModule
{
  LiftCabin lift;
  ...
  public class Button
  {
    ...
    MvSet computePressed ()
    {
      ...
      /***
	  -- the button can be pressed at any point
	  next (pressed) := {true, false}
      ***/
      return next(pressed).eq (top).or (next (pressed).eq (bot));
    }
    ...
    {
      /***
	  -- on reset signal request becomes false
	  -- if a button is pressed request becomes true
	  -- otherwise it stays the same until it is either reset or set
	  next (request) := case
	                     reset  : false;
			     pressed : true;
			     1      : request;
